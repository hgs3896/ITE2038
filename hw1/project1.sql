...
SELECT T.name FROM Trainer AS T, CatchedPokemon AS C WHERE T.id = C.owner_id GROUP BY owner_id HAVING count(*) >= 3;

# 2. 전체 포켓몬 중 가장 많은 2개의 타입을 가진 포켓몬 이름을 사전순으로 출력하세요.(X)
SELECT type FROM Pokemon GROUP BY type ORDER BY name DESC LIMIT 2;
...
# 6. Blue City 출신의 트레이너를 사전순으로 출력하세요. => 트레이너의 이름을 출력하는 것인지?
SELECT name FROM Trainer WHERE hometown = 'Blue City' ORDER BY name;

# 7. 트레이너의 고향 이름을 중복없이 사전순서대로 출력하세요. => 모든 트레이너에 대해 수행인지?
SELECT DISTINCT hometown, name FROM Trainer ORDER BY hometown;

# 8. Red 가잡은포켓몬의평균레벨을출력하세요.
...
SELECT T.name, count(*) FROM Trainer AS T, CatchedPokemon AS C WHERE T.id = C.owner_id and T.hometown = "Sangnok City" ORDER BY count(*);
...
SELECT type, count(*) FROM Pokemon ORDER BY count(*), type GROUP BY type;
...
SELECT T.hometown, AVG(CP.level) FROM CatchedPokemon AS CP, Trainer AS T WHERE CP.owner_id = T.id ORDER BY T.name GROUP BY ;
